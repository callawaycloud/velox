...
# Track all pending threads so we can join them at exit
...
def _flush_pending(timeout: float = 8.0) -> None:
    """Wait for all pending telemetry threads to finish before exit."""
    with _threads_lock:
...
# Register the flush handler so short-lived scripts don't kill threads
