...
* Empty / very short values (<4 chars) are replaced entirely with ``***``.
* Medium values (4-8 chars) show only the last character:
  e.g. ``"mytoken"`` → ``"******n"``
* Longer values (>8 chars) keep the first 2 and last 2 characters with
  stars in between:  e.g. ``AKIAIOSFODNN7EXAMPLE`` → ``AK****************LE``
"""
